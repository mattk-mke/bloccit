...
describe("routes : posts", () => {
  ...
  describe("member user performing CRUD actions for posts", () => {
    ...
    describe("POST /topics/:topicId/posts/:id/destroy", () => {
      it("should not delete a post when the current user is not the owner", done => {
        expect(this.post.id).toBe(1);
        request.post(base + "/" + this.topic.id + "/posts/" + this.post.id + "/destroy", (err, res, body) => {
          Post.findById(1)
          .then( post => {
    ...
    describe("GET /topics/:topicId/posts/:id/edit", () => {
      it("should not render a view with an edit post form if the user is not the owner", done => {
        request.get(base + "/" + this.topic.id + "/posts/" + this.post.id + "/edit", (err, res, body) => {
          expect(err).toBeNull();
          expect(body).not.toContain("Edit Post");
          expect(body).toContain("Snowball Fighting");
  ...
  // Owner user context
  describe("owner user performing CRUD actions for posts that they own", () => {
    beforeEach( done => {
